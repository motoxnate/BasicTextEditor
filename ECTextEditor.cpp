...
void ECTextEditor::ParseKeyCode(int c) {
    ...
    else if(c >= 32 && c <= 126) {  // Letters and characters
    ...
    else if(c >= 0 && c <= 27) { // Special Characters
        switch(c) {
    ...
    else if(c >= 1000) {        // Special keys like arrow keys, del, etc.
        switch(c) {
            case ARROW_LEFT:
...
bool ECTextEditor::SCF() {
    ...
    if(!UpdateCursor(cursorX, cursorY)) return false;
    return true;
}

bool ECTextEditor :: SCB() {
    int cx = textView.GetCursorX() - 1;
    int cy = textView.GetCursorY();
    // If the cursor has reached the beginning of a line.
    if(cx < 0) {
        cy -= 1;
        if(cy < 0) {    // Cursor cannot go back furthers
    ...
    if(!UpdateCursor(cx, cy)) return false;
    return true;
}

bool ECTextEditor :: SCU() {
...
void ECTextEditor :: UpdateTextDisplay() {
    textView.InitRows();
    for(string s : document.GetAllRows()) textView.AddRow(s);
    return;
}
